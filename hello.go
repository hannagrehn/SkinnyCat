...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	// If already starved, only allow quit keys
	if m.starved {
	...
	case tea.KeyMsg:
		...
		// start exercising
		if msg.String() == "e" && m.catWeight >= 20 {
			m.exercising = true
			m.exerciseCount = 0
			m.exerciseStep = 0
			m.question = fmt.Sprintf("%s is exercising! Press %s (%d/%d)", m.catName, "j/k", m.exerciseCount, 10)
			m.err = nil
			return m, nil
		}
